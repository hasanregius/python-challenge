...
f = open('Financial Output.txt', 'w')
with open(csv_path) as revenue_data:
    ...
    f.write('\n')
    f.write(f"Total Revenue: $ {sum(revenue)}")
    f.write('\n')
    f.write(f"Avereage Revenue Change: ${round(avg_rev_change)}")
    f.write('\n')
    f.write(f"Greatest Increase in Revenue: {max_rev_change_date} (${max_rev_change})")
    f.write('\n')
    f.write(f"Greatest Decrease in Revenue: {min_rev_change_date} (${min_rev_change})")
